...
bool compareStudents(const Student& a, const Student& b) {
    ...
    double avgMarkA = (a.mark_of_maths + a.mark_of_physics + ThirdMark(a)) / 3.0;
    double avgMarkB = (b.mark_of_maths + b.mark_of_physics + ThirdMark(b)) / 3.0;

    if (avgMarkA != avgMarkB) {
        
        return avgMarkA > avgMarkB;
    }

    
    int markA = ThirdMark(a);
    int markB = ThirdMark(b);
...
void PrintTable(Student* p, int N, int* indexArray)
{
    cout << "========================================================================================================="
        << endl;
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        cout << "| " << setw(2) << right << i + 1 << " "
            << "| " << setw(14) << left << p[index].prizv
            << "| " << setw(5) << left << p[index].course
            << "| " << setw(5) << left << p[index].spec << " "
            << "| " << setw(11) << left << p[index].mark_of_physics
            << "| " << setw(11) << left << p[index].mark_of_maths;
        switch (p[index].spec)
        {
        case(KOMPNAUKI): cout << "|" << setw(13) << right << p[index].mark_of_3_subj.mark_of_programming << "|            |              |" << endl;; break;
        case(INFORMATIC): cout << "|             |" << setw(11) << right << p[index].mark_of_3_subj.mark_of_chyslmetod << " |              |" << endl;; break;
        default: cout << "|             |            |" << setw(13) << right << p[index].mark_of_3_subj.mark_of_pedagog << " | " << endl; break;
        }
    }
    cout << "========================================================================================================="
        << endl;
